...
The HMAX model consists of fewer layers and only one layer (S2 layer) is trainable, which allows the model to work on limited train samples.
In our proposed model, we combine these two approach.
...
which enables our model to process images of various sizes efficiently. 
...
Currently the model extracts only shape-based features.
Information about color and texture is also very important, which should also participate in the process of visual recognition.
The proposed model is a feed-forward network.
We should also consider the function of feedback connections in the neural systems.
